#010_tradingbot_optimizado_una_operacion.py
import os
import sys
import ccxt.pro
...
class OptimizedTradingBot:
    ...
    def setup_trading_params(self):
        ...
        self.amount = 0.01
        self.positions = {}
        self.current_position = None  # Seguimiento de posición activa
        self.position_side = None  # 'long', 'short', o None
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Verificar posiciones existentes al iniciar
            await self.check_existing_positions()
            logger.info(f"Bot inicializado - Posición actual: {self.position_side}")
        except Exception as e:
            logger.error(f"Error al inicializar: {e}")
            raise

    async def check_existing_positions(self):
        """Verificar si hay posiciones abiertas al iniciar"""
        try:
            positions = await self.hyperliquid.fetch_positions([self.symbol])
            
            for position in positions:
                if position['contracts'] != 0:  # Hay una posición abierta
                    self.current_position = position
                    self.position_side = 'long' if position['side'] == 'long' else 'short'
                    logger.info(f"Posición existente detectada: {self.position_side} - {position['contracts']} contratos")
                    break
            else:
    ...
    def can_open_position(self, side):
        """Verificar si se puede abrir una nueva posición"""
        # No hay posición abierta
        if self.position_side is None:
            return True
        
        # Hay posición abierta del lado opuesto (permite cerrar y abrir nueva)
        if ((self.position_side == 'long' and side == 'sell') or 
            (self.position_side == 'short' and side == 'buy')):
            return True
            
        # Ya hay posición del mismo lado - no permitir
        return False
    ...
    def trading_decision_fast(self, state):
        """Decisión de trading rápida con control de una sola posición"""
        # Evitar decisiones repetitivas
        if state == self.last_decision:
            return 'HOLD'
        
        # Si hay posición abierta, solo permitir cerrarla o mantenerla
        if self.position_side == 'long':
            # Solo permitir vender para cerrar posición long
            if state == 'SELL':
                return 'sell'  # Cerrar posición long
            else:
                return 'HOLD'  # Mantener posición
                
        elif self.position_side == 'short':
            # Solo permitir comprar para cerrar posición short
            if state == 'BUY':
                return 'buy'  # Cerrar posición short
            else:
                return 'HOLD'  # Mantener posición
        
        # No hay posición abierta - permitir abrir nueva
        else:
            decision_map = {
                'BUY': 'buy',
                'SELL': 'sell',
    ...
    async def execute_trade_async(self, order_type, side, amount, price, params):
        """Ejecutar trade de forma asíncrona con control de posición única"""
        try:
            # Verificar si se puede abrir la posición
            if not self.can_open_position(side):
                logger.warning(f"No se puede abrir posición {side} - ya existe posición {self.position_side}")
                return None
            ...
            logger.info(f"Estado anterior: {self.position_side}")
            
            # Actualizar estado de posición
            if self.position_side is None:
                # Abriendo nueva posición
                self.position_side = 'long' if side == 'buy' else 'short'
                logger.info(f"Nueva posición abierta: {self.position_side}")
            else:
                # Cerrando posición existente
                self.position_side = None
                logger.info("Posición cerrada")
            
            self.last_decision = side.upper()
            
            # Actualizar estado de posiciones desde el exchange
            await asyncio.sleep(2)  # Dar tiempo para que se procese la orden
    ...
    async def run_optimized_loop(self):
        ...
        while True:
            try:
                ...
                # Actualizar estado de posiciones periódicamente
                if current_time - last_data_update > 60:  # Cada minuto
                    await self.update_position_status()
                
                logger.info(f"Estado: {current_state}, Decisión: {decision}, Precio: {price_data['last']}, Posición: {self.position_side}")
                
                # Ejecutar trade con cooldown y control de posición única
                if (decision != 'HOLD' and 
                    current_time - last_decision_time > self.DECISION_COOLDOWN):
                    
                    # Verificar si se puede ejecutar la operación
                    if self.can_open_position(decision):
                        ...
                        logger.info(f"Operación {decision} bloqueada - posición {self.position_side} ya abierta")
